...
function BasisWorker() {
    ...
    const basisToEngineMapping = (basisFormat: any, deviceDetails: any) => {
        switch (basisFormat) {
    ...
    const unswizzleGGGR = (data: any) => {
        // given R and G generate B
        const genB = function (R: any, G: any) {
    ...
    const isPOT = (width: number, height: number) => {
    ...
    let basis: any;
    let rgbPriority: any;
    let rgbaPriority: any;

    const chooseTargetFormat = (deviceDetails: any, hasAlpha: any, isUASTC: boolean) => {
        ...
        const testInOrder = (priority: any) => {
    ...
    const dimensionsValid = (width: any, height: any, format: any, webgl2: boolean): boolean => {
        switch (format) {
            // etc1, 2
            case BASIS_FORMAT.cTFETC1:
            case BASIS_FORMAT.cTFETC2:
    ...
    const transcodeKTX2 = (url: any, data: any, options: any) => {
        if (!basis.KTX2File) {
        ...
        return {
            format: basisToEngineMapping(basisFormat as any, options.deviceDetails),
            width: width,
            height: height,
            levels: levelData,
            cubemap: false,
    ...
    const transcodeBasis = (url: any, data: any, options: any) => {
        const funcStart = performanceNow();
    ...
    const transcode = (url: any, data: any, options: any) => {
    ...
    // file url
    const workerTranscode = (url: any, data: any, options: any) => {
        try {
            const result: any = transcode(url, data, options);
            result.levels = result.levels.map((v: any) => v.buffer);
            self.postMessage({url: url, data: result}, result.levels);
        } catch (err) {
            self.postMessage({url: url, err: err}, null as any);
        }
    };

    const workerInit = (config: any, callback: any) => {
        // load the basis file (this is synchronous)
        (self as any).importScripts(config.basisUrl);

        // initialize the wasm module
        const instantiateWasmFunc = (imports: any, successCallback: any) => {
        ...
        (self as any).BASIS(config.module ? {instantiateWasm: instantiateWasmFunc} : null).then((instance: any) => {
