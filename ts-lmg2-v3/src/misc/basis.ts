...
import {BasisWorker} from "./basis-worker";
import {http} from "./http";
import {Logger} from "./logger";

// get the list of the device's supported compression formats
const getCompressionFormats = (device: Engine) => {
    return {
        astc: !!device.extensions.extCompressedTextureASTC,
        atc: !!device.extensions.extCompressedTextureATC,
        dxt: !!device.extensions.extCompressedTextureS3TC,
        etc1: !!device.extensions.extCompressedTextureETC1,
        etc2: !!device.extensions.extCompressedTextureETC,
        pvr: !!device.extensions.extCompressedTexturePVRTC
    };
};

// download basis code and compile the wasm module for use in workers
const prepareWorkerModules = (config: any, callback: any) => {
    const getWorkerBlob = () => {
    ...
    const sendResponse = (basisCode: any, module: any) => {
        callback(null, {
    ...
    if (config.glueUrl && config.wasmUrl && wasmSupported()) {
        let basisCode: any = null;
        let module: any = null;

        // download glue script
        http.get(config.glueUrl, {responseType: "blob"}, (err: any, response: any) => {
            if (err) {
                callback(err);
            } else {
                if (module) {
        ...
        if (WebAssembly.compileStreaming) {
            WebAssembly.compileStreaming(fetchPromise)
                ...
                .catch(err => {
                    Logger.Warn(`compileStreaming() failed for ${config.wasmUrl} (${err}), falling back to arraybuffer download.`);
                    compileManual();
                });
        } else {
            compileManual();
        ...
        http.get(config.fallbackUrl, {responseType: "blob"}, (err: any, response: any) => {
            if (err) {
                callback(err, null);
            } else {
...
class BasisQueue {
    ...
    enqueueJob(url: any, data: any, callback: any, options: any) {
        if (this.callbacks.hasOwnProperty(url)) {
            ...
            if (this.clients.length > 0) {
                (this.clients.shift() as any).run(job);
            } else {
                this.queue.push(job);
            }
        }
    ...
    handleResponse(url: any, err: any, data: any) {
        ...
        if (err) {
            ...
            if (data.format === TextureFormat.PIXELFORMAT_R5_G6_B5 || data.format === TextureFormat.PIXELFORMAT_R4_G4_B4_A4) {
                // handle 16 bit formats
                data.levels = data.levels.map(function (v: any) {
...
class BasisClient {
    ...
    worker: Worker;
    eager: any;
    constructor(queue: any, config: any, eager: any) {
        this.queue = queue;
 ...
 * 'etc2', 'pvr'.
 */
function basisInitialize(config?: any) {
    if (initializing) {
        // already initializing
        return;
    }

    config = (window as any).basisConfig;
    // if (!config) {
    //     config = lazyConfig || {};
    ...
    if (config.glueUrl || config.wasmUrl || config.fallbackUrl) {
        ...
        prepareWorkerModules(config, (err: any, clientConfig: any) => {
            if (err) {
 ...
 * @ignore
 */
function basisTranscode(device: any, url: any, data: any, callback: any, options: any): boolean {
    basisInitialize();
