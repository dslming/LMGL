...
export class EngineUniform {
    ...
    private _innerUniformName = ["projectionMatrix", "modelViewMatrix", "world", "normalMatrix", "modelMatrix", "vEyePosition", "viewMatrix", "inverseViewTransform"]
    ...
    // public cancleUniform(program: any, obj: iProgramUniforms) {
    //       let textureId = 0
    //       const keys = Object.keys(obj)
    //             for (let i = 0; i < keys.length; i++) {
    //                 const name = keys[i]
    ...
    //                     const texture = <Texture>value
    //                     if (texture && texture.isReady()) {
    //                         this._engine.engineTexture.unbindTexture(texture.glTarget)
    //                         // this._engine.engineUniform.setUniform(program, name, value, type, textureId)
    //                         textureId += 1
    //                     }
    //                 }
