...
export class GeometryBufferRenderer {
    ...
    public isReady(subMesh: SubMesh, useInstances: boolean): boolean {
        ...
        if (material) {
            ...
            if (needUv) {
                ...
                if (mesh.meshGeometry.isVerticesDataPresent(VertexBuffer.UVKind)) {
    ...
    public dispose(): void {
        if (this._resizeObserver) {
            ...
            engine.engineObservable.onResizeObservable.remove(this._resizeObserver);
    ...
    protected _createRenderTargets(): void {
        ...
        this._multiRenderTarget = new MultiRenderTarget("gBuffer",
            { width: engine.engineFramebuffer.getRenderWidth() * this._ratio, height: engine.engineFramebuffer.getRenderHeight() * this._ratio }, count, this._scene,
        ...
        this._multiRenderTarget.onClearObservable.add((engine) => {
            engine.engineDraw.clear(new Color4(0.0, 0.0, 0.0, 1.0), true, true, true);
        });

        this._resizeObserver = engine.engineObservable.onResizeObservable.add(() => {
            if (this._multiRenderTarget) {
                this._multiRenderTarget.resize({ width: engine.engineFramebuffer.getRenderWidth() * this._ratio, height: engine.engineFramebuffer.getRenderHeight() * this._ratio });
            }
        });
        ...
        var renderSubMesh = (subMesh: SubMesh): void => {
            ...
            // Managing instances
            var batch = renderingMesh.meshInstanced._getInstancesRenderList(subMesh._id, !!subMesh.getReplacementMesh());
            ...
            if (this.isReady(subMesh, hardwareInstancedRendering)) {
                ...
                renderingMesh.meshGeometry._bind(subMesh, this._effect, material.fillMode);

                this._effect.setMatrix("viewProjection", scene.sceneMatrix.getTransformMatrix());
                this._effect.setMatrix("view", scene.sceneMatrix.getViewMatrix());

                if (material) {
                    var sideOrientation: Nullable<number>;
                    let instanceDataStorage = (effectiveMesh as Mesh).meshInstanced._instanceDataStorage;
