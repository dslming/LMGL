...
export class PrePassRenderer {
    ...
    private _createCompositionEffect() {
        this.prePassRT = new MultiRenderTarget("sceneprePassRT", { width: this._engine.engineFramebuffer.getRenderWidth(), height: this._engine.engineFramebuffer.getRenderHeight() }, this.mrtCount, this._scene,
    ...
    public get isSupported() {
        return this._engine._webGLVersion > 1 || this._scene.getEngine().getCaps().drawBuffersExtension;
    }
    ...
    public _afterCameraDraw() {
        if (this._enabled) {
            const firstCameraPP = this._scene.sceneRender.activeCamera && this._scene.sceneRender.activeCamera._getFirstPostProcess();
            if (firstCameraPP && this._postProcesses.length) {
                this._scene.scenePost.postProcessManager._prepareFrame();
            }
            this._scene.scenePost.postProcessManager.directRender(this._postProcesses, firstCameraPP ? firstCameraPP.inputTexture : null);
        }
    }

    private _checkRTSize() {
        var requiredWidth = this._engine.engineFramebuffer.getRenderWidth(true);
        var requiredHeight = this._engine.engineFramebuffer.getRenderHeight(true);
    ...
    private _bindFrameBuffer() {
        if (this._enabled) {
            ...
            if (internalTexture) {
                this._engine.engineFramebuffer.bindFramebuffer(internalTexture);
            }
        }
    }
    ...
    public clear() {
        if (this._enabled) {
            ...
            this._engine.engineDraw.clear(this._scene.sceneRender.clearColor,
                this._scene.sceneRender.autoClear,
                this._scene.sceneRender.autoClearDepthAndStencil,
                this._scene.sceneRender.autoClearDepthAndStencil);

            // Clearing other attachment with 0 on all other attachments
            this._engine.bindAttachments(this._clearAttachments);
            this._engine.engineDraw.clear(this._clearColor, true, false, false);
            this._engine.bindAttachments(this._defaultAttachments);
        }
    }
    ...
    private _enable() {
        ...
        if (this._scene.sceneRender.activeCamera?._postProcesses) {
            for (let i = 0; i < this._scene.sceneRender.activeCamera._postProcesses.length; i++) {
                if (this._scene.sceneRender.activeCamera._postProcesses[i]?.getClassName() === "ImageProcessingPostProcess") {
    ...
    private _bindPostProcessChain() {
        if (this._postProcesses.length) {
            ...
            const pp = this._scene.sceneRender.activeCamera?._getFirstPostProcess();
            if (pp) {
    ...
    private _update() {
        ...
        const camera = this._scene.sceneRender.activeCamera;
        if (!camera) {
            return;
        }

        const postProcesses = (<Nullable<PostProcess[]>>camera._postProcesses.filter((pp:any) => { return pp != null; }));
        ...
        if (!this.enabled) {
            ...
            this._engine.engineFramebuffer.restoreDefaultFramebuffer();
