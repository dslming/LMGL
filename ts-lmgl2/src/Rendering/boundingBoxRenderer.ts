...
import { ISceneComponent, SceneComponentConstants } from "../Scene/sceneComponent";
...
export class BoundingBoxRenderer implements ISceneComponent {
    ...
    constructor(scene: Scene) {
        this.scene = scene;
        scene.sceneComponent._addComponent(this);
    }
    ...
    private _createIndexBuffer(): void {
        var engine = this.scene.getEngine();
        this._indexBuffer = engine.engineVertex.createIndexBuffer([0, 1, 1, 2, 2, 3, 3, 0, 4, 5, 5, 6, 6, 7, 7, 4, 0, 7, 1, 6, 2, 5, 3, 4]);
    }
    ...
    public render(renderingGroupId: number): void {
        ...
        var engine = this.scene.getEngine();
        engine.engineState.setDepthWrite(false);
        ...
        for (var boundingBoxIndex = 0; boundingBoxIndex < this.renderList.length; boundingBoxIndex++) {
            ...
            // VBOs
            engine.engineVertex.bindBuffers(this._vertexBuffers, this._indexBuffer, <Effect>this._colorShader.getEffect());

            if (this.showBackLines) {
                // Back
                engine.engineState.setDepthFunctionToGreaterOrEqual();
                this.scene.sceneCatch.resetCachedMaterial();
                this._colorShader.setColor4("color", this.backColor.toColor4());
                this._colorShader.bind(worldMatrix);

                // Draw order
                engine.engineDraw.drawElementsType(Material.LineListDrawMode, 0, 24);
            }

            // Front
            engine.engineState.setDepthFunctionToLess();
            this.scene.sceneCatch.resetCachedMaterial();
        ...
        engine.engineState.setDepthFunctionToLessOrEqual();
        engine.engineState.setDepthWrite(true);
    }
    ...
    public renderOcclusionBoundingBox(mesh: AbstractMesh): void {
        ...
        if (!this._fillIndexBuffer) {
            this._fillIndexBuffer = engine.engineVertex.createIndexBuffer(this._fillIndexData!);
        }
        ...
        engine.engineVertex.bindBuffers(this._vertexBuffers, this._fillIndexBuffer, <Effect>this._colorShader.getEffect());

        engine.engineState.setDepthFunctionToLess();
        this.scene.sceneCatch.resetCachedMaterial();
        this._colorShader.bind(worldMatrix);

        engine.engineDraw.drawElementsType(Material.TriangleFillMode, 0, 36);
