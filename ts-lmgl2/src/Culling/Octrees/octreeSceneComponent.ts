 ...
 * @see https://doc.babylonjs.com/how_to/optimizing_your_scene_with_octrees
 */
AbstractMesh.prototype.createOrUpdateSubmeshesOctree = function (
  maxCapacity = 64,
  maxDepth = 2
): Octree<SubMesh> {
...
export class OctreeSceneComponent {
  ...
  public register(): void {
    this.scene.sceneEventTrigger.onMeshRemovedObservable.add(
      (mesh: AbstractMesh) => {
    ...
    this.scene.sceneEventTrigger.onMeshImportedObservable.add(
      (mesh: AbstractMesh) => {
        const sceneOctree = this.scene.selectionOctree;
        if (sceneOctree !== undefined && sceneOctree !== null) {
  ...
  public getActiveMeshCandidates(): ISmartArrayLike<AbstractMesh> {
    if (this.scene._selectionOctree) {
      var selection = this.scene._selectionOctree.select(
