...
export class BoundingInfo implements ICullable {
  ...
  public centerOn(
    ...
    extend: DeepImmutable<Vector3>
  ): BoundingInfo {
    const minimum = BoundingInfo.TmpVector3[0]
      .copyFrom(center)
      .subtractInPlace(extend);
    const maximum = BoundingInfo.TmpVector3[1]
      .copyFrom(center)
      .addInPlace(extend);
    ...
    this.boundingBox.reConstruct(
      minimum,
      maximum,
      this.boundingBox.getWorldMatrix()
    );
    this.boundingSphere.reConstruct(
      minimum,
      maximum,
      this.boundingBox.getWorldMatrix()
    );
  ...
  public isInFrustum(
    ...
    strategy: number = Constants.MESHES_CULLINGSTRATEGY_STANDARD
  ): boolean {
    let inclusionTest =
  ...
  public get diagonalLength(): number {
    ...
    const diag = boundingBox.maximumWorld.subtractToRef(
      boundingBox.minimumWorld,
      BoundingInfo.TmpVector3[0]
    );
    return diag.length();
  }
