...
import { SceneComponentConstants, ISceneSerializableComponent } from "../Scene/sceneComponent";
...
export class EffectLayerSceneComponent implements ISceneSerializableComponent {
    ...
    private _renderMainTexture(camera: Camera): boolean {
        ...
        if (layers && layers.length > 0) {
            this._previousStencilState = this._engine.engineState.getStencilBuffer();
            for (let effectLayer of layers) {
                if (effectLayer.shouldRender() &&
                    ...
                    if (renderTarget._shouldRender()) {
                        this.scene.sceneRender.incrementRenderId();
    ...
    private _setStencil() {
        ...
        if (this._needStencil) {
            this._engine.engineState.setStencilBuffer(true);
        }
    }
    ...
    private _setStencilBack() {
        ...
        if (this._needStencil) {
            this._engine.engineState.setStencilBuffer(this._previousStencilState);
        }
    }
    ...
    private _drawRenderingGroup(index: number): void {
        if (!this.scene.sceneRender._isInIntermediateRendering() && this._renderEffects) {
            this._draw(index);
        }
    }
}
...
EffectLayer._SceneComponentInitialization = (scene: Scene) => {
    let component = scene.sceneComponent._getComponent(SceneComponentConstants.NAME_EFFECTLAYER) as EffectLayerSceneComponent;
    if (!component) {
