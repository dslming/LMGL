...
export class GlowLayer extends EffectLayer {
    ...
    protected _createTextureAndPostProcesses(): void {
        ...
        this._mainTexture.onAfterUnbindObservable.add(() => {
            ...
            if (internalTexture) {
                ...
                this._engine.engineFramebuffer.unBindFramebuffer(internalTexture2 ?? internalTexture, true);
            }
        });
    ...
    protected _internalRender(effect: Effect): void {
        ...
        var previousStencilBuffer = engine.engineState.getStencilBuffer();
        ...
        // Draw order
        engine.engineState.setStencilBuffer(previousStencilBuffer);
    }
    ...
    public serialize(): any {
        ...
        if (this._includedOnlyMeshes.length) {
            for (index = 0; index < this._includedOnlyMeshes.length; index++) {
                var mesh = this._scene.sceneNode.getMeshByUniqueID(this._includedOnlyMeshes[index]);
                if (mesh) {
        ...
        if (this._excludedMeshes.length) {
            for (index = 0; index < this._excludedMeshes.length; index++) {
                var mesh = this._scene.sceneNode.getMeshByUniqueID(this._excludedMeshes[index]);
                if (mesh) {
    ...
    public static Parse(parsedGlowLayer: any, scene: Scene, rootUrl: string): GlowLayer {
        ...
        for (index = 0; index < parsedGlowLayer.excludedMeshes.length; index++) {
            var mesh = scene.sceneNode.getMeshByID(parsedGlowLayer.excludedMeshes[index]);
            if (mesh) {
        ...
        for (index = 0; index < parsedGlowLayer.includedMeshes.length; index++) {
            var mesh = scene.sceneNode.getMeshByID(parsedGlowLayer.includedMeshes[index]);
            if (mesh) {
