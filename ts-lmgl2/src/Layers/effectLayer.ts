...
export abstract class EffectLayer {
    ...
    private _generateIndexBuffer(): void {
        ...
        this._indexBuffer = this._engine.engineVertex.createIndexBuffer(indices);
    }
    ...
    private _setMainTextureSize(): void {
        if (this._effectLayerOptions.mainTextureFixedSize) {
            ...
            this._mainTextureDesiredSize.width = this._engine.engineFramebuffer.getRenderWidth() * this._effectLayerOptions.mainTextureRatio;
            this._mainTextureDesiredSize.height = this._engine.engineFramebuffer.getRenderHeight() * this._effectLayerOptions.mainTextureRatio;

            this._mainTextureDesiredSize.width = this._engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(this._mainTextureDesiredSize.width, this._maxSize) : this._mainTextureDesiredSize.width;
            this._mainTextureDesiredSize.height = this._engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(this._mainTextureDesiredSize.height, this._maxSize) : this._mainTextureDesiredSize.height;
        }
    ...
    protected _createMainTexture(): void {
        ...
        this._mainTexture.customRenderFunction = (opaqueSubMeshes: SmartArray<SubMesh>, alphaTestSubMeshes: SmartArray<SubMesh>, transparentSubMeshes: SmartArray<SubMesh>, depthOnlySubMeshes: SmartArray<SubMesh>): void => {
            ...
            const previousAlphaMode = engine.engineAlpha.getAlphaMode();

            for (index = 0; index < transparentSubMeshes.length; index++) {
                this._renderSubMesh(transparentSubMeshes.data[index], true);
            }

            engine.engineAlpha.setAlphaMode(previousAlphaMode);
        };

        this._mainTexture.onClearObservable.add((engine: Engine) => {
            engine.engineDraw.clear(this.neutralColor, true, true, true);
        });
    ...
    protected _isReady(subMesh: SubMesh, useInstances: boolean, emissiveTexture: Nullable<BaseTexture>): boolean {
        ...
        if (material) {
            ...
            if (diffuseTexture && (needAlphaTest || needAlphaBlendFromDiffuse)) {
                ...
                if (mesh.meshGeometry.isVerticesDataPresent(VertexBuffer.UV2Kind) &&
                    diffuseTexture.coordinatesIndex === 1) {
                    defines.push("#define DIFFUSEUV2");
                    uv2 = true;
                }
                else if (mesh.meshGeometry.isVerticesDataPresent(VertexBuffer.UVKind)) {
        ...
        // Vertex
        if (mesh.meshGeometry.isVerticesDataPresent(VertexBuffer.ColorKind) && mesh.hasVertexAlpha) {
    ...
    public render(): void {
        ...
        engine.engineState.setState(false);

        // VBOs
        engine.engineVertex.bindBuffers(this._vertexBuffers, this._indexBuffer, currentEffect);

        // Cache
        var previousAlphaMode = engine.engineAlpha.getAlphaMode();

        // Go Blend.
        engine.engineAlpha.setAlphaMode(this._effectLayerOptions.alphaBlendingMode);
    ...
    protected _renderSubMesh(subMesh: SubMesh, enableAlphaMode: boolean = false): void {
        ...
        engine.engineState.setState(material.backFaceCulling, material.zOffset, undefined, reverse);

        // Managing instances
        var batch = renderingMesh.meshInstanced._getInstancesRenderList(subMesh._id, !!replacementMesh);
        if (batch.mustReturn) {
            return;
        }
        ...
        if (this._useMeshMaterial(renderingMesh)) {
            ...
            renderingMesh.meshGeometry._bind(subMesh, this._effectLayerMapGenerationEffect, Material.TriangleFillMode);

            this._effectLayerMapGenerationEffect.setMatrix("viewProjection", scene.sceneMatrix.getTransformMatrix());
