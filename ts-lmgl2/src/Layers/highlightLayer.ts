...
export class HighlightLayer extends EffectLayer {
    ...
    constructor(public name: string, scene: Scene, options?: Partial<IHighlightLayerOptions>) {
        ...
        // Warn on stencil
        // if (!this._engine.engineViewPort.isStencilEnable) {
    ...
    protected _createTextureAndPostProcesses(): void {
        ...
        blurTextureWidth = this._engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(blurTextureWidth, this._maxSize) : blurTextureWidth;
        blurTextureHeight = this._engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(blurTextureHeight, this._maxSize) : blurTextureHeight;
        ...
        this._mainTexture.onAfterUnbindObservable.add(() => {
            ...
            if (internalTexture) {
                this._scene.scenePost.postProcessManager.directRender(
                    this._postProcesses,
                    internalTexture,
                    true);
                this._engine.engineFramebuffer.unBindFramebuffer(internalTexture, true);
            }
    ...
    protected _internalRender(effect: Effect): void {
        ...
        engine.engineState.cacheStencilState();

        // Stencil operations
        engine.engineState.setStencilOperationPass(Constants.REPLACE);
        engine.engineState.setStencilOperationFail(Constants.KEEP);
        engine.engineState.setStencilOperationDepthFail(Constants.KEEP);

        // Draw order
        engine.engineState.setStencilMask(0x00);
        engine.engineState.setStencilBuffer(true);
        engine.engineState.setStencilFunctionReference(this._instanceGlowingMeshStencilReference);

        // 2 passes inner outer
        if (this.outerGlow) {
            effect.setFloat("offset", 0);
            engine.engineState.setStencilFunction(Constants.NOTEQUAL);
            engine.engineDraw.drawElementsType(Material.TriangleFillMode, 0, 6);
        }
        if (this.innerGlow) {
            effect.setFloat("offset", 1);
            engine.engineState.setStencilFunction(Constants.EQUAL);
            engine.engineDraw.drawElementsType(Material.TriangleFillMode, 0, 6);
        }

        // Restore Cache
        engine.engineState.restoreStencilState();
    }
    ...
    public addExcludedMesh(mesh: Mesh) {
        ...
        if (!meshExcluded) {
            this._excludedMeshes[mesh.uniqueId] = {
                mesh: mesh,
                beforeBind: mesh.meshObservable.onBeforeBindObservable.add((mesh: Mesh) => {
                    mesh.getEngine().engineState.setStencilBuffer(false);
                }),
                afterRender: mesh.meshObservable.onAfterRenderObservable.add((mesh: Mesh) => {
                    mesh.getEngine().engineState.setStencilBuffer(true);
                }),
            };
        }
    }
    ...
    public removeExcludedMesh(mesh: Mesh) {
        ...
        if (meshExcluded) {
            if (meshExcluded.beforeBind) {
                mesh.meshObservable.onBeforeBindObservable.remove(meshExcluded.beforeBind);
            }

            if (meshExcluded.afterRender) {
                mesh.meshObservable.onAfterRenderObservable.remove(meshExcluded.afterRender);
            }
        }
    ...
    public addMesh(mesh: Mesh, color: Color3, glowEmissiveOnly = false) {
        ...
        if (meshHighlight) {
            ...
            this._meshes[mesh.uniqueId] = {
                ...
                observerHighlight: mesh.meshObservable.onBeforeBindObservable.add((mesh: Mesh) => {
                    if (this.isEnabled) {
                        if (this._excludedMeshes && this._excludedMeshes[mesh.uniqueId]) {
                            this._defaultStencilReference(mesh);
                        }
                        else {
                            mesh.getScene().getEngine().engineState.setStencilFunctionReference(this._instanceGlowingMeshStencilReference);
                        }
                    }
                }),
                observerDefault: mesh.meshObservable.onAfterRenderObservable.add((mesh: Mesh) => {
                    if (this.isEnabled) {
                        this._defaultStencilReference(mesh);
                    }
                }),
    ...
    public removeMesh(mesh: Mesh) {
        ...
        if (meshHighlight) {
            ...
            if (meshHighlight.observerHighlight) {
                mesh.meshObservable.onBeforeBindObservable.remove(meshHighlight.observerHighlight);
            }

            if (meshHighlight.observerDefault) {
                mesh.meshObservable.onAfterRenderObservable.remove(meshHighlight.observerDefault);
            }
    ...
    private _defaultStencilReference(mesh: Mesh) {
        mesh.getScene().getEngine().engineState.setStencilFunctionReference(HighlightLayer.NormalMeshStencilReference);
    }
        ...
        if (this._meshes) {
            ...
            for (let id in this._meshes) {
                ...
                if (meshHighlight && meshHighlight.mesh) {
                    ...
                    if (meshHighlight.observerHighlight) {
                        meshHighlight.mesh.meshObservable.onBeforeBindObservable.remove(meshHighlight.observerHighlight);
                    }

                    if (meshHighlight.observerDefault) {
                        meshHighlight.mesh.meshObservable.onAfterRenderObservable.remove(meshHighlight.observerDefault);
                    }
                }
            }
            this._meshes = null;
            ...
            for (let id in this._excludedMeshes) {
                ...
                if (meshHighlight) {
                    ...
                    if (meshHighlight.beforeBind) {
                        meshHighlight.mesh.meshObservable.onBeforeBindObservable.remove(meshHighlight.beforeBind);
                    }

                    if (meshHighlight.afterRender) {
                        meshHighlight.mesh.meshObservable.onAfterRenderObservable.remove(meshHighlight.afterRender);
                    }
                }
            }
    ...
    public static Parse(parsedHightlightLayer: any, scene: Scene, rootUrl: string): HighlightLayer {
        ...
        for (index = 0; index < parsedHightlightLayer.excludedMeshes.length; index++) {
            var mesh = scene.sceneNode.getMeshByID(parsedHightlightLayer.excludedMeshes[index]);
            if (mesh) {
        ...
        for (index = 0; index < parsedHightlightLayer.meshes.length; index++) {
            ...
            var mesh = scene.sceneNode.getMeshByID(highlightedMesh.meshId);
