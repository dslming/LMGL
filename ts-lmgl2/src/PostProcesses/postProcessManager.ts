...
export class PostProcessManager {
    ...
    private _buildIndexBuffer(): void {
        ...
        this._indexBuffer = this._scene.getEngine().engineVertex.createIndexBuffer(indices);
    }
    ...
    public _prepareFrame(sourceTexture: Nullable<InternalTexture> = null, postProcesses: Nullable<PostProcess[]> = null): boolean {
        let camera = this._scene.sceneRender.activeCamera;
        if (!camera) {
            return false;
        }
        ...
        if (!postProcesses || postProcesses.length === 0 || !this._scene.scenePost.postProcessesEnabled) {
            return false;
        }
    ...
    public directRender(postProcesses: PostProcess[], targetTexture: Nullable<InternalTexture> = null, forceFullscreenViewport = false, faceIndex = 0, lodLevel = 0, doNotBindFrambuffer = false): void {
        ...
        for (var index = 0; index < postProcesses.length; index++) {
            if (index < postProcesses.length - 1) {
                postProcesses[index + 1].activate(this._scene.sceneRender.activeCamera, targetTexture);
            } else {
                if (targetTexture) {
                    engine.engineFramebuffer.bindFramebuffer(targetTexture, faceIndex, undefined, undefined, forceFullscreenViewport, lodLevel);
                } else if (!doNotBindFrambuffer) {
                    engine.engineFramebuffer.restoreDefaultFramebuffer();
                }
            }
            ...
            if (effect) {
                ...
                engine.engineVertex.bindBuffers(this._vertexBuffers, this._indexBuffer, effect);
        ...
        engine.engineState.setDepthBuffer(true);
    ...
    public _finalizeFrame(doNotPresent?: boolean, targetTexture?: InternalTexture, faceIndex?: number, postProcesses?: Array<PostProcess>, forceFullscreenViewport = false): void {
        ...
        if (postProcesses.length === 0 || !this._scene.scenePost.postProcessesEnabled) {
            return;
        }
        var engine = this._scene.getEngine();
        ...
        for (var index = 0, len = postProcesses.length; index < len; index++) {
            ...
            if (index < len - 1) {
                ...
                if (targetTexture) {
                    engine.engineFramebuffer.bindFramebuffer(targetTexture, faceIndex, undefined, undefined, forceFullscreenViewport);
