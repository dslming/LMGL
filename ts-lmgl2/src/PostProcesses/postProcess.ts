...
export class PostProcess {
    ...
    public set samples(n: number) {
        ...
        this._textures.forEach((texture) => {
            if (texture.samples !== this._samples) {
                this._engine.engineTexture.updateRenderTargetTextureSampleCount(texture, this._samples);
            }
        });
    }
    ...
    */
    ...
    constructor(
        ...
        indexParameters?: any, blockCompilation = false, textureFormat = Constants.TEXTUREFORMAT_RGBA) {
            ...
            if (camera != null) {
                ...
                this._scene.scenePost.postProcesses.push(this);
                this.uniqueId = this._scene.getUniqueId();
            }
            else if (engine) {
                this._engine = engine;
                this._engine.enginePost.postProcesses.push(this);
            }
    ...
    public activate(camera: Nullable<Camera>, sourceTexture: Nullable<InternalTexture> = null, forceDepthStencil?: boolean): InternalTexture {
        ...
        var requiredWidth = ((sourceTexture ? sourceTexture.width : this._engine.engineFramebuffer.getRenderWidth(true)) * <number>this._options) | 0;
        var requiredHeight = ((sourceTexture ? sourceTexture.height : this._engine.engineFramebuffer.getRenderHeight(true)) * <number>this._options) | 0;
        ...
        if (!this._shareOutputWithPostProcess && !this._forcedOutputTexture) {
            ...
            if (this.adaptScaleToCurrentViewport) {
                let currentViewport = engine.engineViewPort.currentViewport;
            ...
            if (needMipMaps || this.alwaysForcePOT) {
                if (!(<PostProcessOptions>this._options).width) {
                    desiredWidth = engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(desiredWidth, maxSize, this.scaleMode) : desiredWidth;
                }

                if (!(<PostProcessOptions>this._options).height) {
                    desiredHeight = engine.engineTexture.needPOTTextures ? EngineTexture.GetExponentOfTwo(desiredHeight, maxSize, this.scaleMode) : desiredHeight;
                }
            }
            ...
            if (this.width !== desiredWidth || this.height !== desiredHeight) {
                ...
                let textureOptions = {
                    ...
                    generateStencilBuffer: (forceDepthStencil || camera._postProcesses.indexOf(this) === 0) && this._engine.engineState.isStencilEnable,
            ...
            this._textures.forEach((texture) => {
                if (texture.samples !== this.samples) {
                    this._engine.engineTexture.updateRenderTargetTextureSampleCount(texture, this.samples);
                }
            });
        }
        ...
        if (this.enablePixelPerfectMode) {
            ...
            this._engine.engineFramebuffer.bindFramebuffer(target, 0, requiredWidth, requiredHeight, this.forceFullscreenViewport);
        }
        else {
            this._scaleRatio.copyFromFloats(1, 1);
            this._engine.engineFramebuffer.bindFramebuffer(target, 0, undefined, undefined, this.forceFullscreenViewport);
        }
        ...
        if (this.autoClear && this.alphaMode === Constants.ALPHA_DISABLE) {
            this._engine.engineDraw.clear(this.clearColor ? this.clearColor : scene.sceneRender.clearColor, scene.scenePost._allowPostProcessClearColor, true, true);
        }
    ...
    public apply(): Nullable<Effect> {
        ...
        this._engine.engineState.setState(false);
        this._engine.engineState.setDepthBuffer(false);
        this._engine.engineState.setDepthWrite(false);

        // Alpha
        this._engine.engineAlpha.setAlphaMode(this.alphaMode);
        if (this.alphaConstants) {
            this.getEngine().engineAlpha.setAlphaConstants(this.alphaConstants.r, this.alphaConstants.g, this.alphaConstants.b, this.alphaConstants.a);
        }
    ...
    public dispose(camera?: Camera): void {
        ...
        let index;
        if (this._scene) {
            index = this._scene.scenePost.postProcesses.indexOf(this);
            if (index !== -1) {
                this._scene.scenePost.postProcesses.splice(index, 1);
            }
        }

        index = this._engine.enginePost.postProcesses.indexOf(this);
        if (index !== -1) {
            this._engine.enginePost.postProcesses.splice(index, 1);
        }
    ...
    public static Parse(parsedPostProcess: any, scene: Scene, rootUrl: string): Nullable<PostProcess> {
        ...
        var camera = scene.sceneNode.getCameraByID(parsedPostProcess.cameraId);
