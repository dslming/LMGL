...
export class PostProcess {
  ...
  public activate(camera: Nullable<Camera>, sourceTexture: Nullable<InternalTexture> = null, forceDepthStencil?: boolean): InternalTexture {
    ...
    if (!this._shareOutputWithPostProcess && !this._forcedOutputTexture) {
      ...
      if (needMipMaps || this.alwaysForcePOT) {
        ...
        if (!(<PostProcessOptions>this._options).height) {
          desiredHeight = engine.engineTexture.needPOTTextures
            ? EngineTexture.GetExponentOfTwo(desiredHeight, maxSize, this.scaleMode)
            : desiredHeight;
        }
      }
    ...
    if (this.autoClear && this.alphaMode === Constants.ALPHA_DISABLE) {
      this._engine.engineDraw.clear(
        this.clearColor ? this.clearColor : scene.sceneRender.clearColor,
        scene.scenePost._allowPostProcessClearColor,
        true,
        true
      );
    }
