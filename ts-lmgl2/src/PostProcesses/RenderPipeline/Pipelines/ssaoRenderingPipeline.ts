    ...
    */
    ...
    private _createBlurPostProcess(ratio: number): void {
        ...
        this._blurHPostProcess.onActivateObservable.add(() => {
            let dw = this._blurHPostProcess.width / this._scene.getEngine().engineFramebuffer.getRenderWidth();
            this._blurHPostProcess.kernel = size * dw;
        });

        this._blurVPostProcess.onActivateObservable.add(() => {
            let dw = this._blurVPostProcess.height / this._scene.getEngine().engineFramebuffer.getRenderHeight();
