 ...
 */
export class DefaultRenderingPipeline extends PostProcessRenderPipeline implements IDisposable {
    ...
    public set imageProcessingEnabled(enabled: boolean) {
        ...
        this._scene.sceneRender.imageProcessingConfiguration.isEnabled = enabled;
    }
    ...
    constructor(name: string = "", hdr: boolean = true, scene: Scene = EngineStore.LastCreatedScene!, cameras?: Camera[], automaticBuild = true) {
        ...
        this.chromaticAberration = new ChromaticAberrationPostProcess("ChromaticAberration", engine.engineFramebuffer.getRenderWidth(), engine.engineFramebuffer.getRenderHeight(), 1.0, null, Texture.BILINEAR_SAMPLINGMODE, engine, false, this._defaultPipelineTextureType, true);
        ...
        this._resizeObserver = engine.engineObservable.onResizeObservable.add(() => {
            this._hardwareScaleLevel = engine.getHardwareScalingLevel();
            this.bloomKernel = this.bloomKernel;
        });

        this._imageProcessingConfigurationObserver = this._scene.sceneRender.imageProcessingConfiguration.onUpdateParameters.add(() => {
            this.bloom._downscale._exposure = this._scene.sceneRender.imageProcessingConfiguration.exposure;

            if (this.imageProcessingEnabled !== this._scene.sceneRender.imageProcessingConfiguration.isEnabled) {
                this._imageProcessingEnabled = this._scene.sceneRender.imageProcessingConfiguration.isEnabled;
                this._buildPipeline();
            }
        });
    ...
    private _setAutoClearAndTextureSharing(postProcess: PostProcess, skipTextureSharing = false) {
        if (this._hasCleared) {
            ...
            this._scene.sceneRender.autoClear = false;
    ...
    private _buildPipeline() {
        ...
        this._scene.sceneRender.autoClear = true;
        ...
        if (this.depthOfFieldEnabled) {
            ...
            if (this._cameras.length > 1) {
                ...
                this._depthOfFieldSceneObserver = this._scene.sceneEventTrigger.onAfterRenderTargetsRenderObservable.add((scene) => {
                    if (this._cameras.indexOf(scene.sceneRender.activeCamera!) > -1) {
                        this.depthOfField.depthTexture = scene.enableDepthRenderer(scene.sceneRender.activeCamera).getDepthMap();
                    }
                });
            }
            else {
                this._scene.sceneEventTrigger.onAfterRenderTargetsRenderObservable.remove(this._depthOfFieldSceneObserver);
        ...
        if (this._imageProcessingEnabled) {
            ...
            if (this._hdr) {
                ...
                this._scene.sceneRender.imageProcessingConfiguration.applyByPostProcess = false;
            }
        ...
        if (this._scene.sceneRender.activeCameras && this._scene.sceneRender.activeCameras.length > 1) {
            this._scene.sceneRender.autoClear = true;
        }
    ...
    public dispose(): void {
        ...
        if (this._resizeObserver) {
            this._scene.getEngine().engineObservable.onResizeObservable.remove(this._resizeObserver);
            this._resizeObserver = null;
        }
        this._scene.sceneRender.imageProcessingConfiguration.onUpdateParameters.remove(this._imageProcessingConfigurationObserver);
        super.dispose();
    }
