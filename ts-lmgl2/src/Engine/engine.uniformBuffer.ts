...
Engine.prototype.createUniformBuffer = function (
  elements: FloatArray
): DataBuffer {
...
Engine.prototype.createDynamicUniformBuffer = function (
  elements: FloatArray
): DataBuffer {
  var ubo = this._gl.createBuffer();
...
Engine.prototype.updateUniformBuffer = function (
  uniformBuffer: DataBuffer,
  elements: FloatArray,
  offset?: number,
  count?: number
  ...
  if (count === undefined) {
    ...
    if (elements instanceof Float32Array) {
      ...
      this._gl.bufferSubData(
        this._gl.UNIFORM_BUFFER,
        0,
        new Float32Array(<number[]>elements).subarray(offset, offset + count)
      );
    }
  }
...
Engine.prototype.bindUniformBuffer = function (
  buffer: Nullable<DataBuffer>
): void {
  this._gl.bindBuffer(
    this._gl.UNIFORM_BUFFER,
    buffer ? buffer.underlyingResource : null
  );
};

Engine.prototype.bindUniformBufferBase = function (
  buffer: DataBuffer,
  location: number
): void {
...
Engine.prototype.bindUniformBlock = function (
  pipelineContext: IPipelineContext,
  blockName: string,
  index: number
): void {
