...
export class SceneNode {
  ...
  public addMesh(newMesh: AbstractMesh, recursive = false) {
    ...
    this.scene.sceneEventTrigger.onNewMeshAddedObservable.notifyObservers(
      newMesh
    );
  ...
  public removeMesh(toRemove: AbstractMesh, recursive = false): number {
    ...
    if (index !== -1) {
      ...
      this.scene.meshes[index] =
        this.scene.meshes[this.scene.meshes.length - 1];
    ...
    this.scene.sceneEventTrigger.onMeshRemovedObservable.notifyObservers(
      toRemove
    );
    if (recursive) {
  ...
  public addTransformNode(newTransformNode: TransformNode) {
    ...
    newTransformNode._indexInSceneTransformNodesArray =
      this.scene.transformNodes.length;
  ...
  public removeCamera(toRemove: Camera): number {
    ...
    this.scene.sceneEventTrigger.onCameraRemovedObservable.notifyObservers(
      toRemove
    );
    return index;
  }
  ...
  public removeMaterial(toRemove: Material): number {
    ...
    if (index !== -1 && index < this.scene.materials.length) {
      if (index !== this.scene.materials.length - 1) {
        const lastMaterial =
          this.scene.materials[this.scene.materials.length - 1];
  ...
  public removeTexture(toRemove: BaseTexture): number {
    ...
    this.scene.sceneEventTrigger.onTextureRemovedObservable.notifyObservers(
      toRemove
    );
  ...
  public addLight(newLight: Light): void {
    ...
    this.scene.sceneEventTrigger.onNewLightAddedObservable.notifyObservers(
      newLight
    );
  }
  ...
  public addCamera(newCamera: Camera): void {
    ...
    this.scene.sceneEventTrigger.onNewCameraAddedObservable.notifyObservers(
      newCamera
    );
  ...
  public addTexture(newTexture: BaseTexture): void {
    ...
    this.scene.sceneEventTrigger.onNewTextureAddedObservable.notifyObservers(
      newTexture
    );
  }
  ...
  public addMaterial(newMaterial: Material): void {
    ...
    this.scene.sceneEventTrigger.onNewMaterialAddedObservable.notifyObservers(
      newMaterial
    );
  }
  ...
  public addGeometry(newGeometry: Geometry): void {
    ...
    if (this.scene.geometriesByUniqueId) {
      this.scene.geometriesByUniqueId[newGeometry.uniqueId] =
        this.scene.geometries.length;
    }
  ...
  public removeGeometry(geometry: Geometry): boolean {
    ...
    if (index !== this.scene.geometries.length - 1) {
      const lastGeometry =
        this.scene.geometries[this.scene.geometries.length - 1];
      if (lastGeometry) {
    ...
    this.scene.sceneEventTrigger.onGeometryRemovedObservable.notifyObservers(
      geometry
    );
    return true;
  }
