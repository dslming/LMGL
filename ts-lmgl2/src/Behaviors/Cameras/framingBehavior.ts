...
export class FramingBehavior implements Behavior<ArcRotateCamera> {
  ...
  public attach(camera: ArcRotateCamera): void {
    ...
    this._onMeshTargetChangedObserver =
      camera.onMeshTargetChangedObservable.add((mesh) => {
        if (mesh) {
  ...
  protected _calculateLowerRadiusFromModelBoundingSphere(
    ...
    if (
      ...
      distance =
        distance < camera.lowerRadiusLimit ? camera.lowerRadiusLimit : distance;
    }

    // Don't exceed the upper radius limit
    if (camera.upperRadiusLimit) {
      distance =
        distance > camera.upperRadiusLimit ? camera.upperRadiusLimit : distance;
    }
