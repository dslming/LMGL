...
export class Node implements IBehaviorAware<Node> {
   ...
   */
  public static AddNodeConstructor(
    type: string,
  ...
  public addBehavior(
    ...
    behavior.init();
    if (this._scene.scenePaddingData.isLoading && !attachImmediately) {
      // We defer the attach when the scene will be loaded
      // this._scene.sceneEventTrigger.onDataLoadedObservable.addOnce(() => {
  ...
  protected _syncParentEnabledState() {
    this._isParentEnabled = this._parentNode
      ? this._parentNode.isEnabled()
      : true;
  ...
  public getChildMeshes(
    ...
    this._getDescendants(results, directDescendantsOnly, (node: Node) => {
      return (
        ...
        (<AbstractMesh>node).cullingStrategy !== undefined
      );
    });
    return results;
  }
