...
export class RenderTargetTexture extends Texture {
  ...
  public render(useCameraPostProcess: boolean = false, dumpForDebug: boolean = false): void {
    ...
    //         let mesh = scene.sceneNode.getMeshByID(id);
    //         if (mesh) {
  ...
  private _prepareRenderingManager(
    currentRenderList: Array<AbstractMesh>,
  ...
  public _bindFrameBuffer(faceIndex: number = 0, layer = 0) {
    ...
    var engine = scene.getEngine();
    if (this._texture) {
      engine.engineFramebuffer.bindFramebuffer(
        this._texture,
