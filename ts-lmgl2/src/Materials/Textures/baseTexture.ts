...
export class BaseTexture extends ThinTexture {
   ...
   */
  @serialize()
  public anisotropicFilteringLevel =
   ...
   */
  constructor(sceneOrEngine: Nullable<Scene | Engine>) {
    super(null);
  ...
  /** @hidden */
  protected _getEngine(): Nullable<Engine> {
    return this._engine;
  }
  ...
  public dispose(): void {
    if (this._scene) {
      ...
      this._scene.sceneEventTrigger.onTextureRemovedObservable.notifyObservers(
        this
      );
      this._scene = null;
    }
    ...
    for (var i = 0; i < textures.length; i++) {
      ...
      if (texture.isReady()) {
        ...
        var onLoadObservable = (texture as any)
          .onLoadObservable as Observable<BaseTexture>;
