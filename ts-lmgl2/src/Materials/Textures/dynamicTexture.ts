...
// import "../../Engines//engine.dynamicTexture";
...
export class DynamicTexture extends Texture {
    ...
    constructor(name: string, options: any, scene: Nullable<Scene> = null, generateMipMaps: boolean, samplingMode: number = Constants.TEXTURE_TRILINEAR_SAMPLINGMODE, format: number = Constants.TEXTUREFORMAT_RGBA, invertY?: boolean) {
        ...
        if (options.getContext) {
            ...
            this._texture = engine.engineTexture.createDynamicTexture(options.width, options.height, generateMipMaps, samplingMode);
        } else {
            ...
            if (options.width || options.width === 0) {
                ...
                this._texture = engine.engineTexture.createDynamicTexture(options, options, generateMipMaps, samplingMode);
            }
        }
    ...
    private _recreate(textureSize: ISize): void {
        ...
        this._texture = this._getEngine()!.engineTexture.createDynamicTexture(textureSize.width, textureSize.height, this._generateMipMaps, this.samplingMode);
    }
    ...
    public update(invertY?: boolean, premulAlpha = false): void {
        this._getEngine()!.engineTexture.updateDynamicTexture(this._texture, this._canvas, invertY === undefined ? true : invertY, premulAlpha, this._format || undefined);
    }
