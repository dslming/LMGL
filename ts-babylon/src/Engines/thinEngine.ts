...
} from "../Materials/Textures/internalTexture";
...
export class ThinEngine {
  ...
  constructor(
    ...
    options = options || {};
    PerformanceConfigurator.SetMatrixPrecision(
      !!options.useHighPrecisionMatrix
    );
    ...
    // Viewport
    const devicePixelRatio = DomManagement.IsWindowObjectExist()
    ...
    console.log(
      `lmgl2 base Babylon.js v${ThinEngine.Version} - ${this.description}`
    );
  }
  ...
  public getHostWindow(): Nullable<Window> {
    ...
    if (
      ...
      this._renderingCanvas.ownerDocument &&
      this._renderingCanvas.ownerDocument.defaultView
    ) {
  ...
  public drawElementsType(
    ...
    var indexFormat = this.engineVertex._uintIndicesCurrentlySet
  ...
  protected _initGLContext(): void {
    ...
    this._caps.textureFloatRender =
      this._caps.textureFloat &&
      this.engineFramebuffer._canRenderToFloatFramebuffer()
        ? true
        : false;
    ...
    this._caps.textureHalfFloatRender =
      this._caps.textureHalfFloat &&
      this.engineFramebuffer._canRenderToHalfFloatFramebuffer();
    // Draw buffers
    if (this._webGLVersion > 1) {
      ...
      if (drawBuffersExtension !== null) {
        ...
        this._gl.drawBuffers =
          drawBuffersExtension.drawBuffersWEBGL.bind(drawBuffersExtension);
        this._gl.DRAW_FRAMEBUFFER = this._gl.FRAMEBUFFER;

        for (var i = 0; i < 16; i++) {
          (<any>this._gl)["COLOR_ATTACHMENT" + i + "_WEBGL"] = (<any>(
    ...
    if (this._webGLVersion > 1) {
      ...
      if (depthTextureExtension != null) {
        ...
        this._gl.UNSIGNED_INT_24_8 =
          depthTextureExtension.UNSIGNED_INT_24_8_WEBGL;
      }
    }
    ...
    if (this._webGLVersion > 1) {
      ...
      var instanceExtension = <ANGLE_instanced_arrays>(
        this._gl.getExtension("ANGLE_instanced_arrays")
      );

      if (instanceExtension != null) {
        this._caps.instancedArrays = true;
        this._gl.drawArraysInstanced =
          instanceExtension.drawArraysInstancedANGLE.bind(instanceExtension);
        this._gl.drawElementsInstanced =
          instanceExtension.drawElementsInstancedANGLE.bind(instanceExtension);
        this._gl.vertexAttribDivisor =
          instanceExtension.vertexAttribDivisorANGLE.bind(instanceExtension);
      } else {
    ...
    if (this._gl.getShaderPrecisionFormat) {
      var vertex_highp = this._gl.getShaderPrecisionFormat(
        this._gl.VERTEX_SHADER,
        this._gl.HIGH_FLOAT
      );
      var fragment_highp = this._gl.getShaderPrecisionFormat(
      ...
      if (vertex_highp && fragment_highp) {
        this._caps.highPrecisionShaderSupported =
          vertex_highp.precision !== 0 && fragment_highp.precision !== 0;
      }
    }
    ...
    // Texture maps
    this.engineTexture._maxSimultaneousTextures =
      this._caps.maxCombinedTexturesImageUnits;
    for (
      let slot = 0;
      slot < this.engineTexture._maxSimultaneousTextures;
      slot++
    ) {
  ...
  private _compileShader(
    ...
    return this._compileRawShader(
      ThinEngine._ConcatenateShader(source, defines, shaderVersion),
      type
    );
  }
  ...
  public _loadFile(
    ...
    let request = ThinEngine._FileToolsLoadFile(
      url,
      onSuccess,
      onProgress,
      useArrayBuffer,
