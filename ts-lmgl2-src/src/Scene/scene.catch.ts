...
export class SceneCatch implements iSceneCatch {
  ...
  public clearCachedVertexData(): void {
    for (var meshIndex = 0; meshIndex < this.scene.sceneNode.meshes.length; meshIndex++) {
      var mesh = this.scene.sceneNode.meshes[meshIndex];
      var geometry = (<Mesh>mesh)._geometry;
      ...
      if (geometry) {
        ...
        for (var vbName in geometry._vertexBuffers) {
          ...
          geometry._vertexBuffers[vbName]._data = null;
        }
      }
    }
  }
