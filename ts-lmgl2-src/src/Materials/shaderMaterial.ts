...
//   effect: null as unknown as Effect,
//   subMesh: null as unknown as Nullable<SubMesh>,
// };
...
export class ShaderMaterial extends Material {
  ...
  private _checkCache(mesh?: Mesh, useInstances?: boolean): boolean {
    if (!mesh) {
      return true;
    }
  ...
  public isReady(mesh?: Mesh, useInstances?: boolean): boolean {
    ...
    if (mesh && mesh._geometry.isVerticesDataPresent(VertexBuffer.ColorKind)) {
    ...
    if (this._cachedDefines !== join) {
      ...
      this._effect = engine.enginePipeline.createEffect(
        shaderName,
  ...
  public bind(world: Matrix, mesh?: Mesh, effectOverride?: Nullable<Effect>): void {
    ...
    ubo.bindToEffect(this._effect as any, "Scene");
    this.bindViewProjection(this._effect as any);

    // ubo.update();
    // this.bindForSubMesh(world, mesh as any, (mesh as any).subMeshes[0]);
  }
