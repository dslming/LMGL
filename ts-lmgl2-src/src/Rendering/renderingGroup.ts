...
export class RenderingGroup {
  ...
  private _opaqueSubMeshes = new SmartArray<Mesh>(256);
  private _transparentSubMeshes = new SmartArray<Mesh>(256);
  private _alphaTestSubMeshes = new SmartArray<Mesh>(256);
  private _depthOnlySubMeshes = new SmartArray<Mesh>(256);
  ...
  public render(
    customRenderFunction: Nullable<
      (
        opaqueSubMeshes: SmartArray<Mesh>,
        transparentSubMeshes: SmartArray<Mesh>,
        alphaTestSubMeshes: SmartArray<Mesh>,
        depthOnlySubMeshes: SmartArray<Mesh>
      ) => void
    >,
    renderSprites: boolean,
    renderParticles: boolean,
    activeMeshes: Nullable<Mesh[]>
  ): void {
   ...
   */
  // private renderOpaqueSorted(subMeshes: SmartArray<Mesh>): void {
   ...
   * @param subMeshes The submeshes to render
   */
  // private renderAlphaTestSorted(subMeshes: SmartArray<Mesh>): void {
   ...
   * @param subMeshes The submeshes to render
   */
  // private renderTransparentSorted(subMeshes: SmartArray<Mesh>): void {
  ...
  //   subMeshes: SmartArray<Mesh>,
  //   sortCompareFn: Nullable<(a: Mesh, b: Mesh) => number>,
   ...
   * @param subMeshes The submeshes to render
   */
  private static renderUnsorted(subMeshes: SmartArray<Mesh>): void {
    for (var subIndex = 0; subIndex < subMeshes.length; subIndex++) {
      let mesh = subMeshes.data[subIndex];
      mesh.render();
    }
  }
   ...
   */
  public dispatch(mesh: Mesh, material?: Nullable<Material>): void {
    ...
    if (material.needAlphaBlendingForMesh(mesh)) {
      // Transparent
      this._transparentSubMeshes.push(mesh);
    } else if (material.needAlphaTesting()) {
      // Alpha test
      if (material.needDepthPrePass) {
        this._depthOnlySubMeshes.push(mesh);
      }

      this._alphaTestSubMeshes.push(mesh);
    } else {
      if (material.needDepthPrePass) {
        this._depthOnlySubMeshes.push(mesh);
      }
      ...
      // Opaque
      this._opaqueSubMeshes.push(mesh);
    }
