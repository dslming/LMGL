...
export class GraphNode extends EventHandler {
   ...
   * @private
   */
  _notifyHierarchyStateChanged(node: GraphNode, enabled: boolean) {
   ...
   * @private
   */
  _cloneInternal(clone: GraphNode) {
    clone.name = this.name;

    const tags: any[] = this.tags._list;
    clone.tags.clear();
   ...
   */
  find(attr: Function | string, value?: object): GraphNode[] {
    let result,
      results = [];
    ...
    if (attr instanceof Function) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (descendants.length) results = results.concat(descendants as any);
      }
    } else {
      let testValue;

      if ((this as any)[attr]) {
        if ((this as any)[attr] instanceof Function) {
          testValue = (this as any)[attr]();
        } else {
          testValue = (this as any)[attr];
        }
   ...
   */
  findOne(attr: Function | string, value?: object): GraphNode | null {
    const len = this._children.length;
    let result = null;
  ...
  findByTag(query: any): GraphNode[] {
    ...
    const queryNode = (node: GraphNode, checkNode: boolean) => {
  ...
  findByPath(path: string | string[]): GraphNode | null {
    ...
    for (let i = 0, imax = parts.length; i < imax; ++i) {
      result = result.children.find((c: any) => c.name === parts[i]);
      if (!result) {
        return null;
      }
    }
   ...
   * @private
   */
  _debugInsertChild(node: GraphNode) {
   ...
   * @private
   */
  _fireOnHierarchy(name: string, nameHierarchy: string, parent: GraphNode) {
   ...
   */
  lookAt(x: Vec3 | number, y: Vec3 | number, z: number, ux: number = 0, uy: number = 1, uz: number = 0) {
    if (x instanceof Vec3) {
      target.copy(x);
   ...
   */
  translate(x: Vec3 | number, y: number, z: number) {
    if (x instanceof Vec3) {
      position.copy(x);
    } else {
      position.set(x, y, z);
   ...
   */
  translateLocal(x: Vec3 | number, y: number, z: number) {
    if (x instanceof Vec3) {
      position.copy(x);
    } else {
      position.set(x, y, z);
   ...
   */
  rotate(x: Vec3 | number, y: number, z: number) {
    if (x instanceof Vec3) {
   ...
   */
  rotateLocal(x: Vec3 | number, y: number, z: number) {
    if (x instanceof Vec3) {
      rotation.setFromEulerAngles(x.x, x.y, x.z);
    } else {
      rotation.setFromEulerAngles(x, y, z);
