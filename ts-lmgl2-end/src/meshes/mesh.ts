...
import { BoundingSphere } from '../shape/bounding.sphere';
import {
...
import { Material, MaterialBlend } from './materials/material';
...
export class Mesh {
   ...
   */
  constructor(mesh: Geometry, material: Material) {
    if (!mesh.vertexBuffer) {
      Logger.Error("geometry fail...");
      return;
    }
     ...
     * @type {GraphNode}
     */
    this.node = new GraphNode(); // The node that defines the transform of the mesh instance
  ...
  get aabb() {
    ...
    let localAabb: any = this._customAabb;
  ...
  set material(material) {
    ...
    if (this._material) {
      ...
      const prevBlend = prevMat && prevMat.blendType !== MaterialBlend.BLEND_NONE;
      const thisBlend = this._material.blendType !== MaterialBlend.BLEND_NONE;
  ...
  destroy() {
    ...
    // this.setRealtimeLightmap(Mesh.lightmapParamNames[0], null);
    // this.setRealtimeLightmap(Mesh.lightmapParamNames[1], null);
  ...
  static _prepareRenderStyleForArray(meshInstances: Mesh[], renderStyle:number) {
    if (meshInstances) {
  ...
  updateKey() {
    ...
    this._key[SORTKEY_FORWARD] = getKey(
      this.layer,
      material.alphaToCoverage || material.alphaTest ? MaterialBlend.BLEND_NORMAL : material.blendType, // render alphatest/atoc after opaque
      false,
      material.id
    );
  }
   ...
   */
  setInstancing(vertexBuffer: VertexBuffer | null) {
    if (vertexBuffer) {
   ...
   * in.
   */
  setParameter(name: string, data: number | number[], passFlags: number = -262141) {
...
function getKey(layer: number, blendType: any, isCommand: boolean, materialId: number) {
    // Key definition:
    // Bit
    // 31      : sign bit (leave)
    // 27 - 30 : layer
    ...
    return ((layer & 0x0f) << 27) | ((blendType === MaterialBlend.BLEND_NONE ? 1 : 0) << 26) | ((isCommand ? 1 : 0) << 25) | ((materialId & 0x1ffffff) << 0);
}
