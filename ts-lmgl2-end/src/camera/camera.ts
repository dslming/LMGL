...
import { GraphNode } from '../meshes/graph.node';
...
class Camera {
  ...
  _calculateProjection: null;
  _calculateTransform: null;
  _clearColor: any;
  ...
  constructor() {
    ...
    this._nearClip = 0.1;
    this._node = new GraphNode("camera");
   ...
   */
  worldToScreen(worldCoord: Vec3, cw: number, ch: number, screenCoord: Vec3 = new Vec3()): Vec3 {
   ...
   */
  screenToWorld(x: number, y: number, z: number, cw: number, ch: number, worldCoord = new Vec3()) {
  ...
  getScreenSize(sphere: { center: Vec3; radius: number; }) {
    if (this._projection === PROJECTION_PERSPECTIVE) {
      ...
      const screenViewHeight = Math.tan((this._fov / 2) * MathTool.DEG_TO_RAD);
    ...
    // ortho
    return MathTool.clamp(sphere.radius / this._orthoHeight, 0, 1);
  }
}
