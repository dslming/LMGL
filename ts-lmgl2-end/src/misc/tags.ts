...
class Tags extends EventHandler {
  ...
  add(tags: string | string[]) {
    let changed = false;
    // const tags = this._processArguments(param, true);
   ...
   */
  has(param: string | any[]) {
    if (!this._list.length) return false;

    return this._has(param);
  }
  ...
  //  * @private
  //  */
  // _processArguments(args: string | IArguments | any[], flat?: boolean | undefined) {
  //     const tags:any[] = [];
  //     let tmp = [];
