...
class BoundingBox {
   ...
   */
  constructor(center: Vec3 = new Vec3(), halfExtents: Vec3 = new Vec3(0.5, 0.5, 0.5)) {
    Logger.Assert(!Object.isFrozen(center), `The constructor of 'BoundingBox' does not accept a constant (frozen) object as a 'center' parameter`);
    Logger.Assert(!Object.isFrozen(halfExtents), `The constructor of 'BoundingBox' does not accept a constant (frozen) object as a 'halfExtents' parameter`);
   ...
   */
  setFromTransformedAabb(aabb: BoundingBox, m: Mat4, ignoreScale: boolean = false) {
   ...
   */
  compute(vertices: number[] | Float32Array, numVerts: number) {
  ...
  _distanceToBoundingSphereSq(sphere: { center: { [x: string]: any } }) {
    const boxMin: Vec3 = this.getMin();
    const boxMax: Vec3 = this.getMax();
    ...
    for (let i = 0; i < 3; ++i) {
      ...
      const bMin = (boxMin as any)[axis[i]];
      const bMax = (boxMin as any)[axis[i]];
      let val = 0;
  ...
  _expand(expandMin: Vec3, expandMax: Vec3) {
