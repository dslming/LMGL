...
class VertexIteratorAccessor {
  index: number;
  numComponents: any;
  array: Float32Array | Int8Array | Uint8Array | Int16Array | Uint16Array | Int32Array | Uint32Array;
  stride: number;
  /**
   ...
   * inside the buffer.
   */
  constructor(buffer: ArrayBufferLike, vertexElement: iVertexElement, vertexFormat: { interleaved: any; vertexCount: number }) {
    this.index = 0;
    ...
    switch (vertexElement.numComponents) {
      case 1:
        this.set = this.set1;
        this.getToArray = this.arrayGet1;
        this.setFromArray = this.arraySet1;
        break;

      case 2:
        this.set = this.set2;
        this.getToArray = this.arrayGet2;
        this.setFromArray = this.arraySet2;
        break;

      case 3:
        this.set = this.set3;
        this.getToArray = this.arrayGet3;
        this.setFromArray = this.arraySet3;
        break;

      case 4:
        this.set = this.set4;
        this.getToArray = this.arrayGet4;
        this.setFromArray = this.arraySet4;
        break;
    }
  }
  ...
  set4(a: number, b: number, c: number, d: number) {
    this.array[this.index] = a;
    this.array[this.index + 1] = b;
    this.array[this.index + 2] = c;
  ...
  arraySet1(index: number, inputArray: { [x: string]: number }, inputIndex: string | number) {
    this.array[index] = inputArray[inputIndex];
  }

  arraySet2(index: number, inputArray: { [x: string]: number }, inputIndex: number) {
    this.array[index] = inputArray[inputIndex];
    this.array[index + 1] = inputArray[inputIndex + 1];
  }

  arraySet3(index: number, inputArray: { [x: string]: number }, inputIndex: number) {
    this.array[index] = inputArray[inputIndex];
    this.array[index + 1] = inputArray[inputIndex + 1];
    this.array[index + 2] = inputArray[inputIndex + 2];
  }

  arraySet4(index: number, inputArray: { [x: string]: number }, inputIndex: number) {
    this.array[index] = inputArray[inputIndex];
    this.array[index + 1] = inputArray[inputIndex + 1];
    this.array[index + 2] = inputArray[inputIndex + 2];
  ...
  arrayGet1(offset: number, outputArray: { [x: string]: number }, outputIndex: string | number) {
    outputArray[outputIndex] = this.array[offset];
  }

  arrayGet2(offset: number, outputArray: { [x: string]: number }, outputIndex: number) {
    outputArray[outputIndex] = this.array[offset];
    outputArray[outputIndex + 1] = this.array[offset + 1];
  }

  arrayGet3(offset: number, outputArray: { [x: string]: number }, outputIndex: number) {
    outputArray[outputIndex] = this.array[offset];
    outputArray[outputIndex + 1] = this.array[offset + 1];
    outputArray[outputIndex + 2] = this.array[offset + 2];
  }

  arrayGet4(offset: number, outputArray: { [x: string]: number }, outputIndex: number) {
    outputArray[outputIndex] = this.array[offset];
    outputArray[outputIndex + 1] = this.array[offset + 1];
    outputArray[outputIndex + 2] = this.array[offset + 2];
   ...
   */
  getToArray(offset: any, outputArray: any, outputIndex: any) {
    // Will be replaced with specialized implementation based on number of components
  }
   ...
   */
  setFromArray(index: any, inputArray: any, inputIndex: any) {
    // Will be replaced with specialized implementation based on number of components
  }
}
...
class VertexIterator {
  vertexBuffer: any;
  vertexFormatSize: any;
  buffer: any;
  accessors: Array<VertexIteratorAccessor>;
  element: {
   ...
   * @ignore
   */
  writeData(semantic: string | number, data: number[], numVertices: number) {
    const element = this.element[semantic];
    if (element) {
      if (numVertices > this.vertexBuffer.numVertices) {
        Logger.Error(`NumVertices provided to setData: ${numVertices} is larger than space in VertexBuffer: ${this.vertexBuffer.numVertices}`);
      ...
      if (this.vertexBuffer.getFormat().interleaved) {
        ...
        if (data.length > numVertices * numComponents) {
          ...
          if (ArrayBuffer.isView(data)) {
            data = (data as any).subarray(0, copyCount);
   ...
   * @ignore
   */
  readData(semantic: string | number, data: any[]) {
    const element = this.element[semantic];
    let count = 0;
    if (element) {
